...
const AssetsDetail: React.FC = () => {
  ...
  const fetchDetailData = async () => {
    ...
    try {
      ...
      console.error('Varlık detayları yüklenirken hata:', err);
  ...
  useEffect(() => { if (id) fetchDetailData(); }, [id]);

  useEffect(() => {
    if (!assetInfo?.lastUpdated) return;
    const updateTime = () => {
      const diffSecs = Math.floor((new Date().getTime() - new Date(assetInfo.lastUpdated).getTime()) / 1000);
      const diffMins = Math.floor(diffSecs / 60);
      if (diffSecs < 15) setLiveRelativeTime(t('time_now'));
      else if (diffSecs < 60) setLiveRelativeTime(t('time_sec_ago').replace('{count}', String(diffSecs)));
      else setLiveRelativeTime(t('time_min_ago').replace('{count}', String(diffMins)));
    };
    updateTime();
  ...
  const handleConfirmDelete = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      alert(t('detail_err_server_delete') + (err?.message || ''));
    } finally {
    ...
    <div className="p-8 font-inter max-w-6xl mx-auto min-h-screen bg-[var(--bg-page)] text-[var(--text-main)] transition-colors duration-300">
      <div className="flex flex-col gap-4 mb-4">
        <div className="flex items-start justify-between">
          ...
          <div className="flex flex-col gap-2 items-end">
            <Button variant="filter" className="w-[130px] h-[35px] text-sm shadow-md" onClick={() => navigate(-1)}>
              {t('detail_back_btn')}
            </Button>
            <Button variant="add" className="w-[130px] h-[35px] text-sm shadow-md" onClick={() => setIsAddTransactionOpen(true)}>
              {t('detail_add_tx_btn')}
            </Button>
          </div>
        </div>
      ...
      <div className="asset-detail-grid mb-6">
        {summaryCards.map((card, i) => (
          <div key={i} style={{ backgroundColor: card.bg }} className="border border-[var(--border-color)] rounded-[15px] h-44 flex flex-col items-center justify-center p-4 text-center shadow-sm transition-transform hover:scale-[1.02]">
      ...
      <div className="mb-6">
        <AIAssetInsights assetId={assetInfo.id} assetName={assetInfo.assetName} balance={totalValue} currency={currencySymbol} />
      </div>
            ...
            <tbody>
              {transactions.length === 0 ? (
                ...
                transactions.map((row, index) => {
                  const rowBg = (index + 1) % 2 === 0 ? 'var(--table-row-even-yellow)' : 'var(--table-row-odd-yellow)';
                  const formattedDate = formatToDisplayDate(row.date) || new Date(row.date).toLocaleDateString('tr-TR');

                  return (
                    <tr key={row.id} style={{ backgroundColor: rowBg }} className="h-14 border-b border-[var(--border-color)] last:border-0 text-sm text-[var(--text-main)] hover:brightness-95 transition-all">
                      ...
                      <td className="text-center">
                        <div className="flex justify-center items-center gap-4">
                          <button type="button" onClick={() => { setEditingTx(row); setIsTransactionEditModalOpen(true); }} className="hover:scale-125 transition-transform text-xl" title={t('detail_edit_title')}>📝</button>
                          <span className="text-[var(--text-muted)]">|</span>
                          <button type="button" onClick={() => { setTxToDelete(row.id); setIsConfirmDeleteOpen(true); }} className="hover:scale-125 transition-transform text-xl" title={t('detail_delete_title')}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      <AssetTransactionModal assetId={id ?? ''} isOpen={isAddTransactionOpen} onClose={() => setIsAddTransactionOpen(false)} onCreated={fetchDetailData} />

      <VarlikModallari 
        isOpen={isAssetEditModalOpen} 
        onClose={() => setIsAssetEditModalOpen(false)} 
        mode="edit"
        initialData={assetInfo ? { date: '', assetName: assetInfo.assetName, assetType: assetInfo.assetType as any, amount: assetInfo.totalQuantity, price: avgCost } : undefined}
        onSave={handleUpdateAsset} 
      />

      <VarlikModallari 
        isOpen={isTransactionEditModalOpen} 
        onClose={() => { setIsTransactionEditModalOpen(false); setEditingTx(null); }}
        mode="edit"
        initialData={editingTx ? { date: editingTx.date.split('T')[0], amount: editingTx.totalQuantity, price: editingTx.pricePerUnit, assetType: editingTx.transactionType as any } : undefined}
        onSave={handleUpdateTransaction}
      />

      {isConfirmDeleteOpen && (
        <BaseModal title={t('detail_modal_delete_title')} onClose={() => setIsConfirmDeleteOpen(false)}>
          <div className="p-6 text-center font-inter text-[var(--text-main)] rounded-b-xl">
            <p className="mb-6 text-lg font-medium text-[var(--text-main)]">{t('detail_modal_delete_text')}</p>
            <div className="flex flex-col sm:flex-row justify-center items-center gap-3 sm:gap-4">
              <Button variant="cancel" className="w-full sm:w-[120px] h-[40px] shadow-sm" onClick={() => setIsConfirmDeleteOpen(false)}>
                {t('detail_modal_cancel')}
              </Button>
              <Button variant="apply" className="w-full sm:w-[120px] h-[40px] !bg-red-600 !text-white shadow-sm hover:!bg-red-700" onClick={handleConfirmDelete}>
