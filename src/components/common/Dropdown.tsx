...
const Dropdown: React.FC<DropdownProps> = ({ options, onSelect, placeholder, value }) => {
  ...
  return (
    <div className="relative w-full">
      ...
      >
        <span className="px-4 text-sm text-[#333D50]">
        ...
        <div className="flex h-[35px] w-[35px] items-center justify-center border-l border-[#CDCDCD] bg-white mr-[4px]">
          <ChevronDown 
            size={18} 
            className={`text-[#333D50] transition-transform ${isOpen ? 'rotate-180' : ''}`} 
          />
        </div>
      </div>

      {isOpen && (
        <div className="absolute z-50 mt-2 w-full max-h-[120px] overflow-y-auto border border-[#CDCDCD] bg-white shadow-lg rounded-[6px]">
          {options.map((opt) => (
            <div
              key={opt}
              onClick={() => {
