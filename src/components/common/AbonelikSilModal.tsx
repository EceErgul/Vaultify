...
const AbonelikSilModal = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <BaseModal title="Abonelik Sil" onClose={onClose}>
      <div className="flex flex-col font-inter pr-2">
        
        <div className="flex items-center gap-2 mb-3 ml-2">
          <CustomCheckbox 
            checked={selectedIds.length === mockAbonelikler.length}
            onChange={() => {
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#FF7B7B] text-black text-xs h-10">
                <th className="w-10 border border-black/20"></th>
                <th className="p-2 border border-black/20 text-left font-bold">Abonelik Adı</th>
                <th className="p-2 border border-black/20 font-bold text-center">Ödeme Tarihi</th>
                <th className="p-2 border border-black/20 font-bold text-center">Fiyat</th>
                <th className="p-2 border border-black/20 font-bold text-center">Başlangıç Tarihi</th>
              </tr>
            </thead>
            <tbody>
              {mockAbonelikler.map((sub, index) => {
                ...
                return (
                  <tr 
                    ...
                    className="text-[11px] h-9 border-b border-black/10"
                  >
                    <td className="flex justify-center items-center h-9">
                      <CustomCheckbox 
                    ...
                    <td className="p-2 border-x border-black/10 font-medium">{sub.ad}</td>
